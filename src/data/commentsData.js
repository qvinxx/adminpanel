...
export const useCommentColumns = () => {
  ...
  return [
    ...
    { field: 'customer', headerName: 'Reviewed By', width: isMobile ? 140 : 180,
      renderCell: (params) => (
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
    ...
    { field: 'product', headerName: 'Product', width: isMobile ? 150 : 200,
      renderCell: (params) => (
        <Box sx={{ display: 'flex', alignItems: 'center',}}>
    ...
    },
     {
      ...
      renderCell: (params) => (
        <Box sx={{ display: 'flex', gap: 1 }}>
          <Tooltip title="View">
            <IconButton onClick={() => orderHandlers.handleView(params.row.id)}>
              <VisibilityIcon fontSize="small" color="info" />
            </IconButton>
          </Tooltip>
          <Tooltip title="Edit">
            <IconButton onClick={() => orderHandlers.handleEdit(params.row.id)}>
              <EditIcon fontSize="small" color="primary" />
            </IconButton>
          </Tooltip>
          <Tooltip title="Delete">
            <IconButton onClick={() => orderHandlers.handleDelete(params.row.id)}>
